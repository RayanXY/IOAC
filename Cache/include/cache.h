...
class Memory {
	...
	public:
		int wordsPerBlocks, memorySize, mapping, setSize, replacementPolicy;
...
class Cache : public Memory {
		...
		void showCache (Cache &cache, Memory &memory) {
			...
			for (size_t i = 0; i < cache.elements.size (); i++){
				for (int j = 0; j < cache.wordsPerBlocks; j++){
					std::cout << i << "\t" << cache.elements[i][j]/memory.wordsPerBlocks << "\t" << cache.elements[i][j] << std::endl;
				}
				std::cout << std::endl;
			}
	...
	void changeBlocks (Memory &memory, Cache &cache, int blockCache, int blockMemory) {
		for (int i = 0; memory.wordsPerBlocks; i++) {
